...
| # | Bug | Detalle |
|---|-----|---------|
| ~~8~~ | ✅ **"CORS" en `enviar-push`** | Cerrado 2026-07-30. **No era CORS.** Eran dos causas encadenadas: (a) `usuarioIdDePrestador()` leía `perfiles`, cuya RLS solo permite la propia fila → devolvía `null` y la notificación se saltaba en silencio, así que la función ni se invocaba; (b) el slug real de la Edge Function es `bright-service`, no `enviar-push` (el nombre del dashboard se cambió pero el slug no se puede renombrar) → 404 en el preflight, que Chrome reporta como error de CORS. |
| 8b | **Unificar el slug de la Edge Function** | Cosmético. En el dashboard figura como `enviar-push` pero su URL es `/functions/v1/bright-service`. Para unificar hay que crear una función nueva con el slug correcto, pegar `index.ts`, migrar los secrets `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT`, verificar y borrar la vieja. |
| ~~9~~ | ✅ **400 en loop** | Cerrado 2026-07-30 (v81). `mostrarBannerPrimerTrabajoPro()` consultaba con `prestador_id` en `null` (cuentas `tipo='prestador'` sin ficha) → `prestador_id=eq.` malformado → 400. Traía además dos bugs silenciosos por leer `data.length` en una respuesta con `head:true`: el banner nunca se mostraba, y el aviso de "primer trabajo" salía en **cada** trabajo cerrado. |
...
- ~~**Estadísticas por tier**~~ — ✅ Hecho 2026-07-30 (v85, v86). El campo `estadisticas` estaba en los 4 planes pero no se usaba en ningún lado: la analítica completa se veía sin importar el plan. Ahora cada sección lleva `data-tier` y `aplicarTierEstadisticas()` la muestra según el plan, usando `planParaLimites()` — la misma regla que los límites, así en etapa fundadora Base recibe el tier de Plus. **Export**: descarga el historial de **trabajos** (fecha, rubro, cliente, monto, calificación), no las métricas de analítica — un CSV de "vistas al perfil" no sirve para contabilidad ni para mostrar trayectoria, que son los usos reales. CSV con separador `;` y BOM para que abra bien en Excel es-AR. **PDF no se implementó**: el CSV cubre contabilidad y análisis; el PDF solo agregaría valor para "mostrar trayectoria" con formato, y no está claro que alguien lo pida.
...
- **"CORS error" en Chrome casi nunca es CORS.** Es el mensaje genérico para cualquier preflight que no devuelva headers CORS válidos — incluido un 404 (ruta inexistente) o un 401 (gateway rechazando). Antes de tocar headers, mirar el **status del preflight** en la pestaña Network: eso distingue las causas en un vistazo. Perseguir "el bug de CORS" costó tres hipótesis equivocadas.
- **Una consulta bloqueada por RLS devuelve 200 con resultado vacío, no un error.** Por eso `usuarioIdDePrestador()` fallaba mudo: `data` era `null` y el llamador lo trataba como "no hay destinatario". Cualquier `.eq()` sobre una fila ajena en una tabla con RLS de lectura propia tiene esta trampa.
- **Dos bugs encadenados se esconden entre sí.** El push tenía dos causas independientes; con la primera activa, arreglar la segunda no habría mostrado ningún cambio, y viceversa. Cuando un fix "no hace nada", vale sospechar que hay algo más arriba en la cadena antes de descartarlo.
